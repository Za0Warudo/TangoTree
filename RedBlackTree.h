 ...
 * contains only operations such search, removeMin, removeMax and some extra
 ...
 * can be either RED or BLACK. Could use a boolean instead, but using an enum
...
struct Node {
  ...
  Node(int k)
      : key(k), left(nullptr), right(nullptr), color(RED), blackHeight(1),
        depth(0), minDepth(0), maxDepth(0), isExternal(false) {}
};
 ...
 * it is considered an external node. The depth, minDepth, maxDepth, and
 * blackHeight of the nil node are all set to 0. This allows us to treat the nil
 * node as a leaf in the tree, which simplifies the logic for the operations.
 *
 */
Node *Node::nil = [] {
...
/**
 * @brief Join two Red-Black Trees and a node into a single tree. The first tree
 * contains all the nodes with keys less than the given node's key, the second
 * tree contains all the nodes with keys greater than the given node's key. As a
